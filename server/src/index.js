...
Your purpose:
- Assist students and faculty using ONLY institution-approved knowledge and tools.
...
// Inline auth server logic (previously server/src/auth.js)
...
const STUDENT_EMAIL = /^[A-Za-z]+\.\d{2}[A-Za-z]+@kongu\.edu$/i;
const FACULTY_EMAIL = /^[A-Za-z]+\.[A-Za-z]+@kongu\.edu$/i;
const SERVER_ACCESS = {
...
function detectRole(email) {
  ...
  if (STUDENT_EMAIL.test(normalized)) return "STUDENT";
  if (FACULTY_EMAIL.test(normalized)) return "FACULTY";
  return null;
}
...
// No tool blocking per-role; always return the provided tool list.
function filterToolsForRole(_role, tools) {
  return tools;
}
...
function allowedMcpServersForRole(role) {
  const enabled = new Set(listEnabledMcpServers());
  const roleList = SERVER_ACCESS[role] || Array.from(enabled);
  return roleList.filter((name) => enabled.has(name));
}
...
// Auth endpoints merged from auth.js
...
app.post("/auth/login", async (req, res) => {
  ...
  if (!role) {
    return res.status(400).json({ ok: false, error: "Invalid email format for student or faculty" });
  }

  try {
    const normalizedEmail = String(email).trim().toLowerCase();
    const result = await AUTH_POOL.query(
    ...
    return res.json({
      ok: true,
      role,
      allowedServers: SERVER_ACCESS[role],
...
You must never say that you searched the web, checked online sources, or fetched external data. Such capabilities do not exist.`;
    ...
    if (role === "FACULTY" && mcpToolPolicy && typeof mcpToolPolicy === "object") {
...
You MUST call at least one tool on each listed server before answering. Do not stop after the first server.
If one server returns no data, continue with the remaining servers. Combine the findings into one concise answer.
Only say you lack data after checking all listed servers and finding no relevant information.`;
    }
...
For this ${roleLabel} session, you may use these MCP servers: ${allowedServers.join(",")}.
You MUST attempt to call the relevant tools on each listed server before answering. Do not stop after the first server.
If one server returns no data, continue with the remaining servers. Combine the findings into one concise answer.
Only say you lack data after checking all listed servers.`;
    }
