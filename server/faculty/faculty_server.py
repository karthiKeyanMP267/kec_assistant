...
async def chroma_query_documents(
    ...
    Returns:
        Dictionary with query results including reranked documents, metadatas, and scores
    """
    if not query_texts:
    ...
    try:
        ...
        def _matches_filters(metadata: Dict) -> bool:
            if where:
                # Equality-only filter support for common metadata keys
        ...
        results = {
            "query": query_text,
            "answer": str(response),
            "documents": [[node.text for node in limited_nodes]],
            "metadatas": [[node.metadata for node in limited_nodes]],
            "scores": [[float(node.score) if node.score is not None else None for node in limited_nodes]],
        }
        return results
    except Exception as e:
...
async def chroma_search_by_text_with_limit(
    ...
    """Advanced search with MMR retrieval and optional score filtering.
    ...
    try:
        ...
        nodes = [node for node in response.source_nodes if _matches_filters(node.metadata)]
        ...
        filtered_nodes = [node for node in nodes if _score_in_range(node.score)]
        limited_nodes = filtered_nodes[:n_results]
