...
from document_retriver import build_query_engine, load_index
...
_chroma_client = None
_query_engine = build_query_engine(load_index())
...
async def query_syllabi(
    ...
    """
    try:
        response = await _query_engine.aquery(query)

        def _matches_filters(metadata: Dict) -> bool:
            if filter_department and metadata.get("department") != filter_department:
                return False
            if filter_level and metadata.get("level") != filter_level:
                return False
            if filter_program and metadata.get("program_type") != filter_program:
                return False
            return True

        filtered_nodes = [node for node in response.source_nodes if _matches_filters(node.metadata)]
        limited_nodes = filtered_nodes[:n_results]
        ...
        for node in limited_nodes:
            formatted_results.append({
                ...
                "department": node.metadata.get("department", "N/A"),
                "level": node.metadata.get("level", "N/A"),
                "program_type": node.metadata.get("program_type", "N/A")
            })
        ...
        raise Exception(f"Failed to query syllabi (MMR): {str(e)}") from e
...
async def search_by_department(
    ...
    """
    try:
        query_text = query or department

        def _matches_department(metadata: Dict) -> bool:
            return metadata.get("department") == department

        response = await _query_engine.aquery(query_text)
        filtered_nodes = [node for node in response.source_nodes if _matches_department(node.metadata)]
        limited_nodes = filtered_nodes[:n_results]
        ...
        raise Exception(f"Failed to search by department (MMR): {str(e)}") from e
...
async def peek_collection(
    ...
    try:
        ...
        return {
            ...
            "samples": {
                "ids": _to_serializable(results.get("ids", [])),
                "documents": _to_serializable(results.get("documents", [])),
                "metadatas": _to_serializable(results.get("metadatas", [])),
            },
        }
    except Exception as e:
