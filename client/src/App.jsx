...
const RESTRICTED_TERMS = ['faculty', 'teacher', 'staff', 'professor', 'instructor', 'hod', 'dean']

function friendlyErrorMessage(rawError, userText) {
  const errText = String(rawError || '').trim()
  const errLower = errText.toLowerCase()
  const userLower = String(userText || '').toLowerCase()
  const mentionsRestricted = RESTRICTED_TERMS.some((term) => userLower.includes(term))
  const backendDenied = /(access\s*denied|forbidden|unauthorized|restricted|faculty content)/.test(errLower)

  if (mentionsRestricted || backendDenied) {
    return 'Access restricted: faculty-only content is not available for this account.'
  }
...
function App({ storagePrefix = 'kec' }) {
    ...
    try {
      ...
      for await (const evt of readNdjsonLines(reader)) {
        ...
        if (evt?.type === 'assistant_error' && evt.error) {
          ...
          const friendly = friendlyErrorMessage(msg, text)
          setError('')
          setMessages((m) => {
            const next = [...m]
            if (next[assistantIdx]) next[assistantIdx] = { ...next[assistantIdx], text: friendly }
            return next
          })
        }
        if (evt?.type === 'error') {
      ...
      if (e.name === 'AbortError') {
        ...
        const friendly = friendlyErrorMessage(e?.message || e, text)
        setError('')
        setMessages((m) => {
          const next = [...m]
          if (next[assistantIdx]) next[assistantIdx] = { ...next[assistantIdx], text: friendly }
